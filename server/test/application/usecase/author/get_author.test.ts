...
let author: AuthorAuth;

beforeEach(async () => {
  const repoFac = await useDatabaseContainer("get-author");

  authorRepo = repoFac.author;

  jwtService = new JsonwebtokenJwtService("batman");
  useCase = new GetUser(authorRepo, jwtService);

  author = await AuthorAuth.create("johndoe@gmail.com", "John Doe", "robin");
